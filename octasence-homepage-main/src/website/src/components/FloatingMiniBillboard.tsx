...
const getAirQualityIcon = (category: string) => {
  ...
  switch (level) {
    case 'good':
      return <IconGood {...iconProps} />;
    case 'moderate':
      return <IconModerate {...iconProps} />;
    case 'unhealthy-sensitive-groups':
      return <IconUnhealthy {...iconProps} />;
    case 'unhealthy':
      return <IconUnhealthy {...iconProps} />;
    case 'very-unhealthy':
      return <IconVeryUnhealthy {...iconProps} />;
    case 'hazardous':
      return <IconHazardous {...iconProps} />;
    default:
      return <IconNoValue {...iconProps} />;
  }
};
...
export default function FloatingMiniBillboard({
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] block max-w-[calc(100vw-2rem)]">
      ...
      >
        <div
          className={`bg-gradient-to-br from-blue-600 to-blue-700 text-white rounded-lg shadow-2xl p-4 w-64 hover:scale-105 transition-all duration-300 relative ${isTransitioning ? 'opacity-0' : 'opacity-100 animate-fade-in'
            }`}
        >
          {/* Header Section */}
